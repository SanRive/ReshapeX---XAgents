 ...
 * aqui es que no era literal — que es exactamente lo que el validador degrada a
 * `missing`.
 */
 ...
 *  El orden importa: la mas larga gana cuando dos se solapan. */
export function evidenceStrings(spec: ProjectSpec): string[] {
  const seen = new Set<string>();
  for (const value of Object.values(spec)) {
    if (!value || Array.isArray(value)) continue;
    const field = value as { status?: string; evidence?: string | null };
